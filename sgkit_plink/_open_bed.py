...
class open_bed: #!!!cmk need doc strings everywhere
    def __init__(self, filename, count_A1=True, iid=None, sid=None, pos=None, skip_format_check=False): #!!!document these new optionals. they are here
        ...
        if iid is not None:
            self._row = PstData._fixup_input(iid,empty_creator=lambda ignore:np.empty([0,2],dtype='str'),dtype='str')#!!!cmk need code and coverage
        if sid is not None:
            self._col = PstData._fixup_input(sid,empty_creator=lambda ignore:np.empty([0],dtype='str'),dtype='str')#!!!cmk need code and coverage
        if pos is not None:
            self._col_property = PstData._fixup_input(pos,count=len(self._col),empty_creator=lambda count:np.array([[np.nan, np.nan, np.nan]]*count))#!!!cmk need code and coverage
        ...
        if not hasattr(self,"_col") or not hasattr(self,"_col_property"):
            self._col, self._col_property = self._read_map_or_bim(self.filename,remove_suffix="bed", add_suffix="bim")
    ...
    def _open_file(self):
        ...
        if mode != b'\x01': raise Exception('only SNP-major is implemented') #!!!cmk should mention this
    ...
    def _read(self, iid_index_or_none, sid_index_or_none, order, dtype, force_python_only, view_ok):
        ...
        iid_count_in = self.iid_count#!!!cmk do we really need all six of these values?
    ...
    def read(
        ...
        return self._read(iid_index,sid_index,order,dtype,force_python_only,view_ok)
...
if __name__ == "__main__":
    ...
    import doctest
    doctest.testmod(optionflags=doctest.ELLIPSIS) #!!!cmk how do you doctest with PyTest?
